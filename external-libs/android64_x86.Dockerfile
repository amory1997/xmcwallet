...
## Install wget (tbc)
RUN set -x &&  apt install wget aria2 -y
...
RUN set -x \
    && aria2c ${HTTP_PROXY:+--all-proxy="${HTTP_PROXY}"} -x 16 -s 16 "https://github.com/openssl/openssl/releases/download/openssl-3.0.5/openssl-3.0.5.tar.gz" \ 
# "https://github.com/openssl/openssl/releases/download/openssl-3.0.5/openssl-3.0.5.tar.gz" \  
...
RUN set -x \
    ...
    && CMAKE_INCLUDE_PATH="${PREFIX}/include" \
       ...
       PATH=${HOST_PATH} make FLAVOUR=monero-classic-v4 release-static-android-x86_64-wallet_api -j${NPROC}  MANUAL_SUBMODULES=1
